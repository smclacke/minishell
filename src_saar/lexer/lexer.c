...
/*   Updated: 2023/10/05 17:47:09 by smclacke      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static t_parser	*make_token_list(char **parsed_input)
{
	...
	while (parsed_input[i])
	{
		...
		if (!token_str)
			mini_error("malloc error make_token_lst1", STDERR_FILENO);
		token = lexer_listnew(token_str);
		if (!token)
			mini_error("malloc error make_token_lst2", STDERR_FILENO);
...
t_parser	*lexer(char *input)
{
	...
	if (!parsed_input)
		mini_error("malloc error lexer", STDERR_FILENO);
